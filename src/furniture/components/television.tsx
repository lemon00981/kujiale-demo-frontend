...
function Three({ f }: FurnitureRenderProps) {
  ...
  const panelH = h * 0.72            // 屏幕面板高度
  const panelY = standH + panelH / 2 // 屏幕面板中心高度
  return (
    <group>
      {/* 屏幕面板（机身） */}
      ...
      {/* 立柱 */}
      <Box pos={[0, standH * 0.45, 0]} size={[0.05, standH * 0.8, 0.05]} color={METAL} material="metal" />
      {/* 底座 */}
      <Box pos={[0, 0.03, 0]} size={[w * 0.55, 0.06, d * 5]} color={METAL} material="metal" />
    </group>
  )
}
...
const television: FurnitureComponentDef = {
  type: 'television',            // 唯一标识，别和已有组件重复
  name: '电视机',
  category: '家电',
